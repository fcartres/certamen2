...
describe('GET /api/reminders', async () => {
	...
	test('Lista recordatorios', async ({ token }) => {
		...
		expect.soft(body?.length, 'Es un arreglo').toBeGreaterThan(0) // Cambiar la expectativa
	})
})
...
describe('POST /api/reminders', async () => {
	...
	test('Crea un recordatorio correctamente', async ({ token }) => {
		...
		expect.soft(Number(createdItem1.createdAt)).toBeTypeOf('number') // Convertir a número
